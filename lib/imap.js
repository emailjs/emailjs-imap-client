...
(function(root, factory) {
    ...
    IMAPClient.prototype._processServerQueue = function(){
        ...
        if(response.tag == "+"){
            if(this._currentCommand.payload.length){
                data = this._currentCommand.payload.shift();
                this.onlog("client", data);
                this.waitDrain = this.socket.send(mimefuncs.toArrayBuffer(data + (!this._currentCommand.payload.length ? "\r\n" : "")).buffer);
            }
    ...
    IMAPClient.prototype._sendRequest = function(){
        ...
        try{
            this._currentCommand.payload = imapHandler.compiler(this._currentCommand.request, true);
        }catch(E){
            return this._onError(E);
        }

        var data = this._currentCommand.payload.shift();
        this.onlog("client", data);
        this.waitDrain = this.socket.send(mimefuncs.toArrayBuffer(data + (!this._currentCommand.payload.length ? "\r\n" : "")).buffer);
